...
def insert_row(email, name, password,phonenumber, dateofbirth, IBAN,amount, user_name=config.mysql_user, mysql_password=config.mysql_password, host_name=config.mysql_host, db_name=config.db_name, table_name=config.table_name):
    # Define our connection string
    conn = MySQLdb.connect(user=user_name, password=mysql_password, host=host_name, database=db_name)
    try:
        cursor = conn.cursor()
        cursor.execute("""insert into %s (email, name, password,phonenumber, dateofbirth, IBAN,amount) values ('%s', '%s', '%s','%s', '%s', '%s',%d); """ %(table_name, email, name, password,phonenumber,dateofbirth,IBAN,amount))
        conn.commit()
    finally:
        conn.close()
    print('Row inserted')
